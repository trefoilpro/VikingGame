...
public class DamageDealer : MonoBehaviour
{
    ...
    private void Update()
    {
        /*if (_canDealDamage)
        {
            ...
            if (Physics.Raycast(transform.position, -transform.up, out hit, _weaponLength, _opponentLayers))
            {
                if (!_hasDealtDamage.Contains(hit.transform.gameObject))
                {
        ...
        if (_canDealDamage)
        {
            ...
            {
                if (!_hasDealtDamage.Contains(collider.gameObject))
                {
